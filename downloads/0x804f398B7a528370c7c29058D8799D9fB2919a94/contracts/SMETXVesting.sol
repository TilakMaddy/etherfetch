    ...
    ReentrancyGuardUpgradeable
{
    ...
    function initialize(IERC20Upgradeable _token) public initializer {
        ...
        token = _token;
        intervalTime = 2629800; // 1 month seconds
    ...
    function addTokenGrant(
        address _recipient,
        uint256 _amount,
        uint256 initialLock, //vesting starts after this holding perios(in months)
        ...
        if (tokenGrants[_recipient][crowdsaleRound].amount == 0) {
            Grant memory grant = Grant({
                startTime: currentTime()
                    .add((_lockDurationInMonths).mul(intervalTime))
                    .add((initialLock).mul(intervalTime)),
                amount: _amount,
